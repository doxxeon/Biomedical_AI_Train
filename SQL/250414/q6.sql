...
SELECT REQUEST_ID, student_name FROM CLASS_REQUEST JOIN STUDENT USING (STUDENT_ID);

-- 2. CLASS_REQUEST 테이블에 대해 JOIN절과 USING을 이용해 CLASS테이블과 결합하여
-- 신청번호(REQUEST_ID)와 과목이름(CLASS_NAME)을 조회하는 쿼리를 작성해보세요.
SELECT REQUEST_ID. CLASS_NAME FROM CLASS_REQUEST JOIN CLASS USING (CLASS_ID);
...
SELECT REQUEST_ID, STUDENT_NAME, CLASS_NAME FROM CLASS_REQUEST JOIN STUDENT USING (STUDENT_ID) JOIN CLASS USING (CLASS_ID);
